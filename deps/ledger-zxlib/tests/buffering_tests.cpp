...
TEST(Buffering, SmallBuffer) {
    ...
    buffering_init(ram_buffer, sizeof(ram_buffer), flash_buffer, sizeof(flash_buffer));
...
TEST(Buffering, SmallBufferMultipleTimes_CheckData) {
    ...
    uint8_t small1[100];
    for (size_t i = 0; i < sizeof(small1); i++) {
        small1[i] = i;
    }
    buffering_append(small1, sizeof(small1));

    uint8_t small2[200];
    for (size_t i = 0; i < sizeof(small2); i++) {
        small2[i] = 100 - i;
    }
    ...
    for (size_t i = 0; i < sizeof(small1) + sizeof(small2); i++) {
        if (i < sizeof(small1)) {
...
TEST(Buffering, NoFlashOnlyRAM) {
    ...
    buffering_init(ram_buffer, sizeof(ram_buffer), nullptr, 0);
