...
//SCO.01
parser_error_t parser_getItemSetupStaingColection(const parser_context_t *ctx,
                                       uint16_t displayIdx,
                                       char *outKey, uint16_t outKeyLen,
                                       char *outVal, uint16_t outValLen,
                                       uint8_t pageIdx, uint8_t *pageCount) {
    *pageCount = 1;
    switch (displayIdx) {
        case 0:
            snprintf(outKey, outKeyLen, "Type");
            snprintf(outVal, outValLen, "Setup Staking Colection");
            return PARSER_OK;
        case 1:
            snprintf(outKey, outKeyLen, "ChainID");
            return parser_printChainID(&parser_tx_obj.payer,
    ...
    switch (parser_tx_obj.script.type) {
        ...
        case SCRIPT_TH24_WITHDRAW_REWARDED_DELEGATED_TOKENS:
            return parser_getItemWithdrawRewardedDelegatedTokens(ctx, displayIdx, outKey, outKeyLen, outVal, 
                                                                 outValLen, pageIdx, pageCount);
        case SCRIPT_SCO01_SETUP_STAKING_COLECTION:
            return parser_getItemSetupStaingColection(ctx, displayIdx, outKey, outKeyLen, outVal, 
                                                                 outValLen, pageIdx, pageCount);
